class StaticPagesController < ApplicationController
  ...
  before_filter :authenticate_user!, :except => [:home, :courses, :contact]

  def home
    @publications = Publication.find(:all, :order => "pub_date DESC", :limit => 3)
    @talks = Talk.find(:all, :order => "talk_date DESC", :limit => 3)
    @content = StaticPage.find_by_name("home").content
  end

  def courses
    @content = StaticPage.find_by_name("courses").content
  end

  def contact
    @content = StaticPage.find_by_name("contact").content
  end
